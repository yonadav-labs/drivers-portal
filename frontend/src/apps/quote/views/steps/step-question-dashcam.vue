<template>
  <quote-process-layout>
    ...
    <quote-process-yes-no-form @yes="yes" @no="no">
      Does your vehicle have a dash cam system? 
...
export default class StepQuestionDashcam extends Vue {
  @quote.Getter
  ...
  yes(): void {
    this.updateQuestionAnswers({ dash_cam: true })
    this.next()
  }

  no(): void {
    this.updateQuestionAnswers({ dash_cam: false })
    this.next()
  }

  resetState(): void {
    this.updateQuestionAnswers({ dash_cam: undefined });
